...
public class ShopManager : MonoBehaviour {
    ...
    public string GetItemDescription(ShopItemScriptableObject shopItem) {
        ...
        if(shopItem.miningRateUpgradeboostAdd != 0)
            result += "Mining Rate <b><color=#4DEA52>" + shopItem.miningRateUpgradeboostAdd.ToString("+#;-#;0") + "</color></b><sprite=4>\n";

        if(shopItem.miningpowerBoostAdd != 0)
            result += "Mining Power <b><color=#4DEA52>" + shopItem.miningpowerBoostAdd.ToString("+#;-#;0") + "</color></b><sprite=5>\n";
        ...
        if(shopItem.miningRateUpgradePercentAdd != 0)
            result += "Mining Rate <b><color=#4DEA52>" + (shopItem.miningRateUpgradePercentAdd*100).ToString("+#;-#;0") + "%</color></b><sprite=4>\n";
    ...
    public void ActiveItemBonus(ShopItemScriptableObject shopItem) {
        ...
        if(shopItem.miningOreReceivedBoostAdd != 0)
            player.miningpower.AddModifier(new Modifier(shopItem.miningOreReceivedBoostAdd, ModifierType.ADDITION));
        ...
        if(shopItem.miningOreReceivedPercentAdd != 0)
            player.miningpower.AddModifier(new Modifier(shopItem.miningOreReceivedPercentAdd, ModifierType.ADD_PERCENTAGE));
    }
}
